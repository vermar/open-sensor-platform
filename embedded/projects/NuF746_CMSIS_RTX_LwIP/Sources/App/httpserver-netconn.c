...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define WEBSERVER_THREAD_PRIO    ( osPriorityIdle + 4 )
...
void http_server_netconn_init()
{
  ...
  sys_thread_new("HTTP", https_server, NULL, DEFAULT_THREAD_STACKSIZE, WEBSERVER_THREAD_PRIO);
#endif
}
...
{
  char PAGE_BODY[530];  //Warning!! This local buffer adds to the task stack usage
  char pagehits[10] = {0};
#ifdef ASF_PROFILING
  int32_t len, len2;
#endif
  ...
  strcat((char *)PAGE_BODY, "<pre><br>Name              Stack Use  Stack(%)  CPU(%)   #Run" );

  /* The list of tasks and their status */
#ifdef ASF_PROFILING
  len = strlen(PAGE_BODY);
  len2 = DoProfiling(false, (char*)(PAGE_BODY + len), sizeof(PAGE_BODY) - len);
  if (len2 >= (sizeof(PAGE_BODY) - len))
  {
      D0_printf(G_RED"Warning! Page buffer maybe truncated!\r\n"G_NORM);
  }
#else
  strcat((char *)PAGE_BODY, "<br>---------------------------------------------<br>");
  osThreadList((unsigned char *)(PAGE_BODY + strlen(PAGE_BODY))); >RKV<
  strcat((char *)PAGE_BODY, "<br><br>---------------------------------------------");
