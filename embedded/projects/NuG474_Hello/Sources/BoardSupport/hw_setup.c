...
void DebugUARTConfig( uint32_t baud, uint32_t dataLen, uint32_t stopBits, uint32_t parity )
{
    ...
    gDbgUartPort.hUart->Init.OneBitSampling = UART_ONE_BIT_SAMPLE_DISABLE;
    gDbgUartPort.hUart->Init.ClockPrescaler = UART_PRESCALER_DIV1;
    gDbgUartPort.hUart->AdvancedInit.AdvFeatureInit = UART_ADVFEATURE_NO_INIT;
    ...
    HAL_UARTEx_SetTxFifoThreshold(gDbgUartPort.hUart, UART_TXFIFO_THRESHOLD_1_8);
    HAL_UARTEx_SetRxFifoThreshold(gDbgUartPort.hUart, UART_RXFIFO_THRESHOLD_1_8);
    HAL_UARTEx_DisableFifoMode(gDbgUartPort.hUart);
}
